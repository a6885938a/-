...
gulp.task('revImg_C', function() {
    gulp.src([N_TY + 'rev/images/*.json', HtmlTY + '*.html']) //- 读取 rev-manifest.json 文件以及需要进行css名替换的文件
        // .pipe(revCollector())     
        .pipe(revCollector({
...
gulp.task('revCss', function() {
    return gulp.src([HtmlTY + 'rev/images/*.json', N_TY + 'css/*.css'])
        .pipe(revCollector())
...
gulp.task('miniHtml', function() {
    setTimeout(function() {
        gulp.src([HtmlTY + 'rev/*/*.json', HtmlTY + '*.html']) //- 读取 rev-manifest.json 文件以及需要进行css名替换的文件
